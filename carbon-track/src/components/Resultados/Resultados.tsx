...
export default function Resultados() {
  ...
  async function fetchResultados() {
    try {
      ...
      setError("Erro ao buscar resultados." + e);
    } 
  }
  ...
  async function handleDelete(id: number) {
    try {
        ...
        setResultados((at) => at.filter((resultado) => resultado.idResultado !== id));
  ...
  return (
    <div className="p-4  rounded-md shadow-md">
      ...
      <ul className="space-y-4">
        {resultados.map(({ idResultado, gastoLuz, gastoGas, resultadoCalculo, dataCalculo }) => (
          <li
            key={idResultado}
            className="flex flex-col bg-primary p-4 rounded-md shadow-sm border "
          >
            <p className="text-white">
              <strong className="text-secondary">Gasto de Luz:</strong> {gastoLuz}
            </p>
            <p className="text-white">
              <strong className="text-secondary">Gasto de Gás:</strong> {gastoGas}
            </p>
            <p className="text-white">
              <strong className="text-secondary">Resultado do Cálculo:</strong> {resultadoCalculo}
            </p>
            <p className="text-white">
